define(function(require, exports){
	...
	exports.config = {
		...
		lang : {
			...
			E01 : 'Server error, please try again later.'
		}
	}
	var cache = {},
	...
	function add(){
		...
		$add.on('click',function(){
			var $tpl = jQuery(config.tpl.replace(/\%placeholder\%/ig,get_random_int(100,999)));
			del($tpl);
	...
	function del($tpl){
		$tpl.find('.delete').on('click',function(){
			I(jQuery(this).data('target')).css('background','#d54e21')
	...
	function check_redeem(){
		...
		function event_click(e){
			...
			xhr.open('get',config.process_url + '&' + tools.param(fd));
			xhr.send();
			xhr.onload = function(){
