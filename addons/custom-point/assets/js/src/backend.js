define(function(require, exports){
	...
	exports.bind = function(){
		...
		cache.$user_id.addEventListener('blur',action_get_point);
		//cache.$user_id.addEventListener('blur',ajax_user_id);
		//cache.$user_id.addEventListener('blur',ajax_user_id);
		cache.$user_set.addEventListener('click',action_user_set);
		
	}
	function action_get_point(){
		var $this = this,
		...
		tip($this,'loading',config.lang.M00001);
		...
		xhr.onload = function(){
			if(xhr.status >= 200 && xhr.status < 400){
				...
				if(data && data.status){
					tip($this,data.status,data.msg);
				}else{
					tip($this,'error',config.lang.E00001);
				}
			}else{
				tip($this,'error',config.lang.E00001);
			}
	...
	function tip(el,t,s){
		...
		var $tip = document.getElementById(el.getAttribute('data-target'));
		$tip.innerHTML = tools.status_tip(t,s);
