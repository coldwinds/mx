...
function hide_menu(){var icon_active=$last_click_btn.getAttribute('data-icon-active'),icon_original=$last_click_btn.getAttribute('data-icon-original');$last_target.classList.remove('on');if(icon_active&&icon_original){$last_click_btn.classList.remove(icon_active);$last_click_btn.classList.add(icon_original);}}
function helper(e){$last_target=Q(this.getAttribute('data-toggle-target'));$last_click_btn=this;if($last_target.classList.contains('on')){hide_menu();}else{show_menu();}}
...
return;function Q(e){return document.querySelector(e);}
var $layer=document.createElement('div');$layer.id='mobile-on-layer';$layer.addEventListener('click',hide_menu);document.body.appendChild($layer);var $last_click_btn,$last_target;function show_menu(){var icon_active=$last_click_btn.getAttribute('data-icon-active'),icon_original=$last_click_btn.getAttribute('data-icon-original');document.body.classList.add('menu-on');$last_target.classList.add('on');if(icon_active&&icon_original){$last_click_btn.classList.remove(icon_original);$last_click_btn.classList.add(icon_active);}
var focus_target=$last_click_btn.getAttribute('data-focus-target');if(focus_target){var $focus_target=Q(focus_target);if($focus_target){$focus_target.focus();}}}
...
function helper(e){$last_target=Q(this.getAttribute('data-mobile-target'));$last_click_btn=this;if($last_target.classList.contains('on')){hide_menu();}else{show_menu();}}
for(var i=0,len=$toggles.length;i<len;i++){$toggles[i].addEventListener('click',helper);}}
