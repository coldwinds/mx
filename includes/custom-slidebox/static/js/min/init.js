...
function candy(){cache.$blurs=cache.$slide.querySelectorAll('.area-blur .item');cache.$mains=cache.$slide.querySelectorAll('.area-main .item');cache.$thumbnails=cache.$slide.querySelectorAll('.area-thumbnail .item');cache.len=cache.$thumbnails.length;cache.current_i=0;function event_hover(e){var current_i=this.getAttribute('data-i');if(cache.current_i==current_i)
