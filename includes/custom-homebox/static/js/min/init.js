...
function append_content_nav(){for(var i=0,len=cache.$boxes.length;i<len;i++){var $title=cache.$boxes[i].querySelector('.mod-title a'),title=$title.textContent,$i=$title.querySelector('i'),offsetTop=getElementTop(cache.$boxes[i])-100,$item=document.createElement('a');if(!$i)
