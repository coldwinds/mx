...
return;cache.len=cache.$boxes.length;cache.$last_boxes=cache.$boxes[cache.len-1];cache.ori_bottom=getElementTop(cache.$last_boxes)+cache.$last_boxes.offsetHeight;cache.ori_offset_left=getElementLeft(document.getElementById('sidebar-container'));cache.ori_offset_top=getElementTop(cache.$boxes[0]);create_nav();bind_window_scroll();}
...
function append_content_nav(){for(var i=0,len=cache.$boxes.length;i<len;i++){var $title=cache.$boxes[i].querySelector('.mod-title a'),title=$title.textContent,icon_class=$title.querySelector('i').getAttribute('class'),offsetTop=getElementTop(cache.$boxes[i])-100,$item=document.createElement('a');cache.offset_top_sets[offsetTop]=i;$item.setAttribute('data-scroll-top',offsetTop);$item.href='#'+cache.$boxes[i].id;$item.title=title;$item.innerHTML='<i class="'+icon_class+' fa-fw"></i>';$item.addEventListener('click',scroll_to);cache.$items[i]=$item;cache.$nav.appendChild(cache.$items[i]);}}
